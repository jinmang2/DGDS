...
class Seq2SeqDataset(AbstractSeq2SeqDataset):
    ...
    def __getitem__(self, index) -> Dict[str, str]:
        ...
        source_line = self.prefix + linecache.getline(str(self.src_file), index).rstrip("\n")
        assert source_line, f"empty source line for index {index}"
        output = {"src_texts": source_line, "id": index - 1}
        if self.type_path != 'test':
            tgt_line = linecache.getline(str(self.tgt_file), index).rstrip("\n")
            assert tgt_line, f"empty tgt line for index {index}"
            output["tgt_texts"] = tgt_line
        return output
    ...
    def collate_fn(self, batch) -> Dict[str, torch.Tensor]:
        ...
        batch_encoding: Dict[str, torch.Tensor] = self.tokenizer.prepare_seq2seq_batch(
            ...
            tgt_texts=[x["tgt_texts"] for x in batch] if self.type_path != "test" else None,
            max_length=self.max_source_length,
            max_target_length=self.max_target_length if self.type_path != "test" else None,
            return_tensors="pt",
            **self.dataset_kwargs,
        ).data
...
class Seq2SeqDataCollator:
    ...
    def __call__(self, batch) -> Dict[str, torch.Tensor]:
        if batch[0].get("tgt_texts", None) is not None:
    ...
    def _encode(self, batch) -> Dict[str, torch.Tensor]:
        batch_encoding = self.tokenizer.prepare_seq2seq_batch(**batch)
